...
static void uart_event_task(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    if (xQueueReceive(rpc_uart_evt_queue, (void *)&event, (portTickType)portMAX_DELAY))
    {
...
static void rpc_uart_rx_task(void *arg)
{
  ...
  while (1)
  {
    if(xQueueReceive(s_espuart_rx_queue, &rx_info, portMAX_DELAY) == pdTRUE)
    {
      ...
      /* Handle data */
      RPCProtocol_dispatchInfo(rx_info.data, rx_info.data_len, \
                                RPCProtocol_ESPUART_dispatchHandler);
      free(rx_info.data);
    }
  }
  vTaskDelete(NULL);
